...
async def create_task_router(task_data: TaskCreate, db: sessionDep, user: authDep_user) -> dict[str, str]:
    await TaskMethods.create_task(task_data, db, user)

    return {"status": "Task is successfully created!"}
...
async def delete_task_router(task_id: int, db: sessionDep, user: authDep_user) -> Task:
    task = await TaskMethods.delete_task_by_id(task_id, db)
...
async def update_task_router(task_id: int, new_data: TaskUpdate, db: sessionDep, user: authDep_user) -> Task:
    task = await TaskMethods.update_task_by_id(task_id, new_data, db)
...
@router.post("/test_tasks")
async def test_tasks_router(db: sessionDep) -> dict[str, str]:
    await TaskMethods.reset_tasks(db)
