...
class UserService:
    ...
    async def get_user_by_id(db: sessionDep, user_id: int) -> UserRelInDB:
        query = (
            ...
            .options(
                selectinload(UserModel.tasks).load_only(
                    TaskModel.id,
                    TaskModel.title,
    ...
    async def get_all_users(db: sessionDep) -> dict[str, list[UserRelInDB]]:
        ...
        return {
            "users": [
                UserRelInDB.model_validate(i, from_attributes=True) for i in records
            ]
        }
    ...
    async def update_user(
        db: sessionDep, user_id: int, new_username: str = "", new_roles: str = ""
    ) -> UserBase:
    ...
    async def delete_user(user_id: int, db: sessionDep) -> HTTPException:
        ...
        if user:
            ...
            await db.commit()
            raise HTTPException(
                status_code=status.HTTP_202_ACCEPTED, detail="User deleted"
            )
        else:
            raise user_not_found_exeption
