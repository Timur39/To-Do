...
task_not_found_exeption = HTTPException(
    status_code=status.HTTP_404_NOT_FOUND, detail="Задач(а) не найдено(а)"
)
...
class TaskService:
    ...
    async def create_task(
        task_data: TaskCreate, db: sessionDep, user: UserModel
    ) -> Task:
        task = TaskModel(
    ...
    @staticmethod
    async def get_task_by_id(task_id: int, db: sessionDep) -> TaskInBD:
    ...
    @staticmethod
    async def delete_task_by_id(task_id: int, db: sessionDep) -> TaskInBD:
        try:
            task = await db.get(TaskModel, task_id)
            if not task:
                raise task_not_found_exeption
    ...
    # Получить все задачи
    @staticmethod
    async def get_all_tasks(db: sessionDep) -> dict[str, list[TaskInBD]]:
    ...
    async def get_user_tasks(user_id: int, db: sessionDep) -> dict[str, list[Task]]:
        ...
        return {
            "tasks": [
                Task.model_validate(task, from_attributes=True) for task in result
            ]
        }
