...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)], 
    ...
    user = await get_user_by_email(db, email=token_data.email) # Получаем пользователя из БД
...
authDep_user = Annotated[User, Depends(get_current_active_user)]
authDep_admin = Annotated[User, Depends(get_current_admin)]
